...
class MyWindow(QWidget):
  ...
  def _tick_video(self):
    ...
    if self.live_or_stored == "stored":
      ...
      dep_ubyte2 = np.array(depfrm_bytes, np.uint16, copy=False).reshape((424, 512))
      dep_ubyte1 = (dep_ubyte2 / 256).astype(np.uint8)
      rgbfrm_raw = np.array(rgbfrm_bytes, np.uint8, copy=False).reshape((1080, 1920, 4))[..., :3]
      ...
      if show_action:
        ...
        if ('위장_' + action_kr_name if tickcwise_flagdict[FLAG_DIM_LIGHT] else action_kr_name) == EVENT_KR_roam:
          self.editor2.appendPlainText("[{start}~{end}] Event : [{act_name}] R_Frame: [{frame_number}] GT_Frame: [{gt_number}]".format(
            start=time_bef.strftime("%H:%M:%S"), end=time_now.strftime("%H:%M:%S"),
            act_name='위장_' + action_kr_name if tickcwise_flagdict[FLAG_DIM_LIGHT] else action_kr_name,
  ...
  def setupUI(self):
    ...
    qimage_color = QImage(im_np_color, im_np_color.shape[1], im_np_color.shape[0], QImage.Format_RGB888)
    pixmap_color = QPixmap(qimage_color)
    pixmap_color = pixmap_color.scaled(800, 550)  
    self.label_color.setPixmap(pixmap_color)

    qimage_depth = QImage(im_np_depth, im_np_depth.shape[1], im_np_depth.shape[0], QImage.Format_RGB888)
    ...
    # 메뉴 컬러 뎁스 스위치
    self.color_depth_switch = SwitchButton(self, "2D Image", 15, "Depth Map Image", 165 - 75, 218)
    swTmp1 = QLabel(" ")
  ...
  def btnclicked_load_storedVideo(self):
    ...
    if "DEP" in str(avipath).upper():
      vidpath_dep = avipath
      vidpath_rgb = Path(str(avipath).replace(DEP_FOLDER_NAME, RGB_FOLDER_NAME))

    elif "RGB" in str(avipath).upper():
      vidpath_rgb = avipath
      vidpath_dep = Path(str(avipath).replace(RGB_FOLDER_NAME, DEP_FOLDER_NAME))
    else:
      self.n_frames = 0
      return
  ...
  def get_tickwise_flagdict(self):
    if self.tickwise_flagdict is None:
      self.tickwise_flagdict = dict(zip(TICKWISE_FLAG_KEYS, [False, ] * len(TICKWISE_FLAG_KEYS)))
