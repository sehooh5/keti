...
def get_align_color_image(kinect, color_img, color_height=1080, color_width=1920, depth_height=424, depth_width=512):
    ...
    kinect._mapper.MapDepthFrameToColorSpace(
        kinect._depth_frame_data_capacity, kinect._depth_frame_data, CSP_Count, CSP)

    colorXYs = np.copy(np.ctypeslib.as_array(CSP, shape=(
        depth_height*depth_width,)))  # Convert ctype pointer to array
...
def get_joint2D(joints, jointPoints):
    ...
    joint2D = np.zeros((PyKinectV2.JointType_Count, 2), dtype=np.int32)
...
def get_joint_quaternions(orientations):
    ...
    quat = np.zeros((PyKinectV2.JointType_Count, 4), dtype=np.float32)
    for i in range(PyKinectV2.JointType_Count):
...
def draw_bone2D(img, j2D, color=(0, 0, 255)):  # Default red lines
    ...
    for i in range(j2D.shape[0]):  # Should loop 25 times
        if j2D[k[i], 0] > 0 and j2D[k[i], 1] > 0 and j2D[i, 0] > 0 and j2D[i, 1] > 0:
            cv2.line(img, (j2D[k[i], 0], j2D[k[i], 1]),
                     (j2D[i, 0], j2D[i, 1]), color)
    ...
    if body_frame is not None:
        for i in range(0, kinect.max_body_count):
            ...
            if body.is_tracked:
                ...
                joint2D = get_joint2D(joints, joint_points)
...
def create_line_set_bones(joints):
    ...
    lines = [[0, 1], [1, 20], [20, 2], [2, 3],  # Spine
             [20, 4], [4, 5], [5, 6], [6, 7], [
                 7, 21], [7, 22],  # Left arm and hand
             [20, 8], [8, 9], [9, 10], [10, 11], [
                 11, 23], [11, 24],  # Right arm and hand
...
def create_color_point_cloud(align_color_img, depth_img,
    ...
    align_color_img = align_color_img[:, :, 0:3]
...
def transform_geometry_quaternion(joint3D, orientation):
    ...
    transform_matrix = [[1 - 2*qy*qy - 2*qz*qz,  2*qx*qy - 2*qz*qw,  2*qx*qz + 2*qy*qw, tx],
                        [2*qx*qy + 2*qz*qw,  1 - 2*qx*qx -
                            2*qz*qz,  2*qy*qz - 2*qx*qw, ty],
                        [2*qx*qz - 2*qy*qw,  2*qy*qz + 2*qx *
                            qw,  1 - 2*qx*qx - 2*qy*qy, tz],
                        [0,  0,  0,  1]]
