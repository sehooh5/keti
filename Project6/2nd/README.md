...
- http://123.214.186.244:9997/index.html
...
- 진행 상황 :
  - 클러스터 생성 : O
  - 클러스터 삭제 : O
    - body 없이 삭제하고 있음 - 삭제는 잘 되는데 지금 변경한 코드 잘 돌아가는지 확인 필요
  - AI 업로드, 수정 : O
    - 기존에 배포 테스트 했던 파일 찾아서 해당 리포지터리에서 업로드 하는 것 진행
    - Master 서버에서의 위치 : 
      - /home/edge-master-01/monitoring.zip
...
- ~~현재 포트 문제 해결중에 외부에서 :5231 API 서버로 요청이 안들어가짐...방화벽 재설정하든지 해서 요청 가능하게 한다음에~~
- ~~호스트명 edge-master-01 등등으로 바꾸고 다시 해보던가 해야할듯 !~~
- **문제 해결 : config 지정 문제**
...
- 에러슈팅 : 
  - AI 업로드 수정 후 개별 배포할 때 docker image 가 변경이 안되는데 왜?
    - zip 파일 해제시 폴더 지정해줘야함
  - k8s 배포기능 업데이트 안됨
    - 기존 배포된 AI 지우고 진행하는거로 다시 코드 작성
    - {sw_name}-{host_name} 의 pod 을 갖고 있으면 삭제하는 코드
...
- ~~연동 되어있는 시스템으로 마스터, 워커 지우고 처음부터 다시 시작해보기~~
- 서버
  - 예
    - API : http://123.214.186.244:4880
    - Web : API : http://123.214.186.244:9997
  - 세
    - Master : http://192.168.0.14
      - API : http://123.214.186.244:5231
    - Worker : http://192.168.0.9
- ~~서버가 죽어서 다시 살려달라해야함~~
