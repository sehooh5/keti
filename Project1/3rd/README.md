...
- ParseData 에서 읽어오는 데이터들 yy,ss 등등 을 전달해야하는데 어떻게??
- cpp 코드를 python 에 바인딩 해서 실행은 가능
  - 문제점 : 
    - 그냥 실행만 시키면 C++코드가 그냥 print out 되는 형태여서 C++에서 사용중인 변수를 python 에서 사용이 불가능
...
- 정리 : 
  ...
  - 다른방법 : 
    - python 의 subprocess 를 사용하여 C++에서 실행되는 것을 사용가능
...
- [구조체 C++ -> Python 성공!!](https://bibliotheque.tistory.com/25) -> gwg5.cpp / gwg5.py 에 작성 완료!!
- 앞으로 할 것 : 
  - 구조체 생성해서 python 에 구조체 전달해야하는데
  - 기존 gwg3 에서 데이터 생성시 계속 while 문이 돌아서 어떻게 생성하고 전달할 지 생각해봐야함 
...
- 어제 코드 해석 후 저장 
- C++ -> Python struct 전달 코드와 기존 gwg코드 합치기
- gwg4 에서 잘 되는듯?!! 확인 후 전달 드리기!!



#### 1220

- gwg4 에서 기존 python 에서 했던 작업들 옮기기



#### 1221

- gwg4 에서 requests.post 로 요청 보낼 시 "세그먼테이션 오류" 메시지 뜸
  - gps 데이터 구조가 c와 python 이 서로 달라서 그랬음
- requests 요청을 하면 에러가 떴는데 대기시간을 1초로 늘려줘서 에어 해결!



#### 1222

- 전체 과제 점검하고 앞으로 해야할 것들 정리
  - DEMS  기능 살리기
  - 무선엣지과제도 확인 후 연동하는거 진행필요
...
- DEMS 디바이스 연결/해제 
  ...
  - 디바이스 연결 해제 테스트
- 문제점 : DEMS 파트에서 수정필요
  - 연결 버튼을 눌렀을 때 내쪽 IP 가 잘못 되어잇음 수정필요
  - 부재중으로 수정이 불가능함
...
- 연동되는 부분 오류나는 것 확인
  - 요청 주소가 잘못됨  29 가 아닌 28로 보내야되서 수정 요청
...
- 오류나는 부분 해결
- 전체 코드 및 gps 연동 확인
  - gps_with_gyro4.py and .cpp 연동 후 
  - 엣지서버로 전송되어 저장됨
...
온-오프라인이 결합된 제품 판매 사업

\- 네이버와 같이 상품판매 사업 관점 리서치 필요

\- o2o 서비스 : https://ko.wikipedia.org/wiki/O2O
...
- 경량화 추가 및 온, 오프라인 커머스 내가 보완할 부분 완료하기(퇴근전까지)
  - 애플 영어 리서치 부분 오후에 추가하기
  - 해외 사례 1개정도 더 추가하기
  - 이후에 온오프라인 시작
