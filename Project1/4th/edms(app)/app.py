...
# 2.4.3 신규 엣지 클러스터 추가 (get_edgeInfo 사용)
...
# 2.5.3 신규 엣지 AI 패키지 추가 인터페이스
@ app.route('/add_newEdgeAi', methods=['POST'])
def add_newEdgeAi():
    func = sys._getframe().f_code.co_name
    print(datetime.datetime.now().strftime(
...
@ app.route('/remove_selectedEdgeAi', methods=['POST'])
def remove_selectedEdgeAi():
    func = sys._getframe().f_code.co_name
    print(datetime.datetime.now().strftime(
...
# 2.5.5 선택 엣지 AI 패키지의 정보 수정 인터페이스
@ app.route('/update_selectedEdgeAiInfo', methods=['POST'])
def update_selectedEdgeAiInfo():
