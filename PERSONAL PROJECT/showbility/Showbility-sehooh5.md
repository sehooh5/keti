...
- User 기능 구현 시작
  ...
  - feat/user branch 에서 작업 진행
    ...
    - 테스트중인데 swagger UI (/docs)로 진행하면됨
        - create 완료
      - delete : auth안되서 못하는중, 토큰 사용해서 다시해야할듯
        - login 기능을 먼저 해야하나?
  ...
  - 추가해야 하는부분
    ...
    - **소셜 로그인 기능 (Kakao, Apple 등)**:
      ...
      **이메일 인증 및 비밀번호 재설정**:
      ...
      - Django에서는 **이메일 인증** 및 **비밀번호 재설정** 기능이 있었습니다.
      - FastAPI에서 이메일 인증과 비밀번호 재설정 기능은 아직 구현되지 않았습니다.
  
      **사용자 삭제 (User Delete)**:
  
      - Django에서는 사용자를 삭제하는 기능이 있었으며, 이는 FastAPI에서도 구현해야 합니다.
      - FastAPI에서 `DELETE /users/{user_id}` 라우트를 추가하여 구현할 수 있습니다.
  
      **사용자 업데이트 (User Update)**:
  
      - Django에서는 사용자 정보를 수정하는 기능이 있었지만, FastAPI에서는 아직 구현되지 않았습니다.
      - 이 기능은 사용자 업데이트를 처리할 **PUT 또는 PATCH** 메서드를 통해 구현해야 합니다.
